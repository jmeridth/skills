...
- **Mark ready when ready label**: When creating a draft PR, find a GitHub Actions workflow in `.github/workflows/` that references the `kenyonj/mark-ready-when-ready` action. Read the trigger label name from that step's `label:` input; if the input is unset, the name is the action's default, "Mark Ready When Ready". Then check that the repository has a label with that exact name (`gh label list --search "<name>"`). If both conditions are met, add that label to the draft PR. If either condition is not met, notify me that the label was not applied and which condition(s) failed.
